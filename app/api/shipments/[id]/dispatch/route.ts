...
export async function POST(
  ...
  try {
    ...
    const legResult = await client.query(
      `
        SELECT
          ...
          from_el.type AS from_location_type,
    ...
    /*
     * Pastikan keberangkatan pertama berasal dari Gudang/Warehouse.
     */
    if (
      leg.from_location_type !== "WAREHOUSE" &&
      Number(leg.from_event_location_id) !== 1
    ) {
      throw new Error(
        "Lokasi keberangkatan shipment bukan Gudang penyimpanan."
      );
    }
